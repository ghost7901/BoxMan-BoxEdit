...
BOOL CDib::Stretch(CDC *pDC,CPoint ptDest,CSize szDest,CPoint ptSrc,CSize szSrc,DWORD dwRop)
{
	if(m_lpBMIH == NULL) return FALSE;
	if(m_hPalette != NULL) {
		::SelectPalette(pDC->GetSafeHdc(), m_hPalette, TRUE);
	...
	pDC->SetStretchBltMode(COLORONCOLOR);
	::StretchDIBits(pDC->GetSafeHdc(), ptDest.x, ptDest.y, szDest.cx, szDest.cy,
		ptSrc.x, ptSrc.y, szSrc.cx, szSrc.cy,
		m_lpImage, (LPBITMAPINFO) m_lpBMIH, DIB_RGB_COLORS, dwRop);
	return TRUE;
}

//BOOL CDib::DrawDib(CDC *pDC,CPoint ptDest,CSize szDest,CPoint ptSrc,CSize szSrc,DWORD dwRop)
//{
//	if(m_lpBMIH == NULL) return FALSE;
//	if(m_hPalette != NULL) {
//		::SelectPalette(pDC->GetSafeHdc(), m_hPalette, TRUE);
...
//	CRect rcDest(ptDest,szDest),
//		  rcSrc(ptSrc,szSrc);
//	pDC->LPtoDP(rcDest);
//	pDC->LPtoDP(rcSrc);
//	rcDest-=pDC->GetViewportOrg();
//	rcSrc-=pDC->GetViewportOrg();
//	int nMapModeOld=pDC->SetMapMode(MM_TEXT);
//	::DrawDibDraw(hdd,pDC->GetSafeHdc(),rcDest.left,rcDest.top,
//		rcDest.Width(),rcDest.Height(),m_lpBMIH,m_lpImage,rcSrc.left,rcSrc.top,
//		rcSrc.Width(),rcSrc.Height(),dwRop);
...
{
	DWORD dwPos;
	dwPos = static_cast <DWORD>(ar.GetFile()->GetPosition());
